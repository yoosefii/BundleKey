...
# class DM(db.Model):                                ##FEATURE UNDER DEVELOPMENT
#     id = db.Column(db.Integer, primary_key=True)
#     chat_id = db.Column(db.Integer)
#     part1_id = db.Column(db.Integer)
#     part2_id = db.Column(db.Integer)
...
# @app.route('/profile/<int:user_id>/<string:operation>', methods=['POST', 'GET'])              ##FEATURE UNDER DEVELOPMENT
@login_required
def profile(user_id=None, operation=None): #why set to None? ah i guess for the first rout we need to define the variable
    if user_id == None:
        user_id = current_user.id
    user = User.query.filter_by(id=user_id).first()
    posts = Post.query.filter_by(poster_id=user_id).order_by(Post.id.desc()).all()
    ...
    comments = Comment.query.filter_by(commenter_id=user_id).order_by(Comment.id.desc()).all()
    ...
    return render_template(
                            # 'DM.html' if operation == 'DM' else                 ##FEATURE UNDER DEVELOPMENT
...
def feed():
    posts = Post.query.order_by(desc(Post.published)).all()
    posts_data = [{'object': post,
                   'time_ago': time_ago(post.published),
                   'likes': json.loads(post.likes_json),
                   'dislikes': json.loads(post.dislikes_json)} for post in posts]
...
def replies_page(comment_id):
    ...
    replies = Reply.query.filter_by(comment_id=comment.id).order_by(Reply.id.desc())
    post_data = {'object': post,
                'time_ago': time_ago(post.published),
                'likes': json.loads(post.likes_json),
                'dislikes': json.loads(post.dislikes_json)}
...
@login_required
def notifications():
    notifs = Notification.query.filter_by(receiver_id=current_user.id).order_by(Notification.id.desc()).all()
    def link(notif):
        if notif.action_type == 'comment':
            obj = Comment.query.filter_by(id=notif.action_id).first()
            return f'{url_for('comments_page', post_id=obj.post_id)}#comment-{obj.id}'
        elif notif.action_type == 'reply':
            obj = Reply.query.filter_by(id=notif.action_id).first()
            return f'{url_for('replies_page', comment_id=obj.comment_id)}#reply-{obj.id}'
    notifs_data = [{'obj':notif,
                    'sender_obj': User.query.filter_by(id=notif.sender_id).first(),
                    'time_ago':time_ago(notif.published),
                    'link':link(notif)} for notif in notifs]
    return render_template('notifications.html', notifs = notifs_data)
...
def delete(type, id:int):
    ...
    if type == 'post':
        ...
        lvl1_notifs_lists = [Notification.query.filter_by(action_type='comment',action_id=comment.id).all() for comment in underlying_objs_lvl1]
        lvl1_notifs = []
        for lst in lvl1_notifs_lists:
            lvl1_notifs.extend(lst)
        underlying_objs_lvl2 = Reply.query.filter_by(post_id=obj.id).all()
        lvl2_notifs_lists = [Notification.query.filter_by(action_type='reply',action_id=reply.id).all() for reply in underlying_objs_lvl2]
        lvl2_notifs = []
        for lst in lvl2_notifs_lists:
            lvl2_notifs.extend(lst)
        to_delete = [obj, *underlying_objs_lvl1, *underlying_objs_lvl2, *lvl1_notifs, *lvl2_notifs]
        target = url_for('feed')
    elif type == 'comment':
        obj = Comment.query.filter_by(id=id).first()
        underlying_objs_lvl1 = Reply.query.filter_by(comment_id=obj.id).all()
        self_notifs = Notification.query.filter_by(action_type='comment',action_id=obj.id).all()
        lvl1_notifs_lists = [Notification.query.filter_by(action_type='reply',action_id=reply.id).all() for reply in underlying_objs_lvl1]
        lvl1_notifs = []
        for lst in lvl1_notifs_lists:
            lvl1_notifs.extend(lst)
        to_delete = [obj, *underlying_objs_lvl1, *self_notifs, *lvl1_notifs]
        target = f'{url_for('comments_page', post_id = obj.post_id)}'
    elif type == 'reply':
        obj = Reply.query.filter_by(id=id).first()
        self_notifs = Notification.query.filter_by(action_type='reply',action_id=obj.id).all()
        to_delete = [obj, *self_notifs]
...
def add_reply(comment_id:int, post_id:int):
    ...
    db.session.flush()
    comment = Comment.query.filter_by(id=comment_id).first()
    notification = Notification(action_type='reply',
                                action_id=reply.id,
                                sender_id=current_user.id,
                                receiver_id=comment.commenter_id)
    db.session.add(notification)
    db.session.commit()
...
if __name__ == '__main__':
    with app.app_context():
        # objs = Notification.query.order_by(Notification.id).all()
        # for obj in objs:
